{if (!$chartEnabled || !$chartSupported )&& $rows}
    ...
    <table width="100%">
        <tr>
            ...
            <td width="20%" style="text-align: right"><img width="150" src="https://crm.bemas.org/sites/default/files/bemas/bemas_logo_200.jpg"></td>
        </tr>
    </table>
    ...
    {capture assign=sectionHeaderTemplate}
        ...
        {foreach from=$sections item=section key=column name=sections}
            ...
            {/if}
            {$l}/if{$r}
            <tr class="crm-report-sectionHeader crm-report-sectionHeader-{$h}"><th colspan="{$columnCount} style="{$bemasColHeaderStyle}">
    ...
    {foreach from=$rows item=row key=rowid}
        ...
        <tr  class="{cycle values="odd-row,even-row"} {$row.class} crm-report" id="crm-report_{$rowid}">
            {foreach from=$columnHeaders item=header key=field}
                ...
                <td style="{$bemasCellStyle} class="crm-report-{$field}{if $header.type eq 1024 OR $header.type eq 1 OR $header.type eq 512} report-contents-right{elseif $row.$field eq 'Subtotal'} report-label{/if}">
                    {if $row.$fieldLink}
